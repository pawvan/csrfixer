 ...
 */
const configureCSR = require('./utils/csr');
const configureWebWorker = require('./utils/webWorker');
const applySeoFixes = require('./utils/seo');
const handleLargeFiles = require('./utils/largeFileHandler');
const getEnvironment = require('./utils/env');
module.exports = function fixCSR(webpackConfig) {
  const env = getEnvironment();
  webpackConfig = configureCSR(webpackConfig, env);
  webpackConfig = configureWebWorker(webpackConfig, env);
  applySeoFixes(env);
  webpackConfig = handleLargeFiles(webpackConfig, env);
